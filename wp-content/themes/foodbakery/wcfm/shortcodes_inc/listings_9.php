    ...
    <script>
        ...
        function ez_get_restaurents(latlng, is_page_loaded) {
            ...
            if (latlng) {
                ...
                the_data.push({name: 'radius', value: 4000});
            }
        ...
        function geocodeAddress(address) {
            ...
            geocoder.geocode({address: address}, (results, status) => {
                if (status === "OK") {
                    ez_get_restaurents(results[0].geometry['location'], null);
                } else {
                    alert(
...
//                                    debugger;
            ...
            jQuery('#the_listings').html('<?php echo loading(); ?>');
...
//    $lng = 33.6676974;
 ...
 if (!empty($lat) && !empty($lng)) {
        /////////////////////Count////////////////////
        $select_count .= " , ( $earth_radius * acos(
        cos( radians( $lat ) )
    ...
    $join_count
    INNER JOIN {$wpdb->prefix}usermeta latitude ON (latitude.user_id = u1.id AND latitude.meta_key = '_wcfm_store_lat')
    INNER JOIN {$wpdb->prefix}usermeta longitude ON (longitude.user_id = u1.id AND longitude.meta_key = '_wcfm_store_lng')
    WHERE $condition 
