                            ...
                            <form id="ez_search_restaurants">
                                ...
                                <div class="cs-rich-editor">
                                    ...
                                    <div class="col-lg-7 col-md-7 col-sm-12 col-xs-12">
                                        <div id="the_listings"></div>
                                        <div class="row" id="the_pagination_"></div>
                                    </div>
                                    <!-- Column Start -->
    ...
    <script>
        ...
        function ez_get_restaurents(obj) {
            ...
            jQuery.ajax({
                ...
                //                jQuery('#the_listings').one().append(response.pagination);
                //                if(is_pagination !='yes'){jQuery('#the_listings').append(response.pagination);}
                jQuery('#the_pagination_').html(response.pagination);
...
//    }
    ...
    $per_page = isset($_REQUEST['per_page']) ? $_REQUEST['per_page'] : 2;
    $current_page = isset($_REQUEST['current_page']) ? $_REQUEST['current_page'] : 1;
//    $offset = (int)$_REQUEST['offset'];
    $offset = (int)isset($_REQUEST['offset']) ? $_REQUEST['offset'] : ($current_page==1?$current_page:1)-1 * $per_page;
    ...
    if ($limit != "") {// IF CLICKED ON PAGINATION LINK I.E TO PAGES 2ND 3RD AND SO ON
        $the_limit = " LIMIT $offset, $limit";
    }else{// IF THERE'S NO PAGINATION LINK I.E: THE FIRST TIME THESE VALUES SHOULD BE LIKE SO:
        $the_limit = " LIMIT 2, 1";
    }
...
//AND (m2.meta_value IS NULL OR m2.meta_value like 'xyz')
    ...
    $restaurants_count = $wpdb->get_results($count_query);
...
//                echo $cuisine;
        ...
        ob_start();
//print_r($restaurants_count);
        echo 'per Page: ' . $per_page;
        echo '  current ' . $current_page;
        echo ' offset ' . $offset = ($current_page==1?$current_page:1)-1 * $per_page;
        
        $limit = $per_page; //($current_page * $per_page);
//        $total_pages = ceil($restaurants_count[0]->total / $per_page);
        ?>           

        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <input type="hidden" name="total_results" id="total_results" value="<?php echo $restaurants_count[0]->total; ?>"/>
            ...
            <div class="page-nation cp_paginate">
                <?php // echo pagination_bar($restaurants_count[0]->total, $per_page); ?>
                <?php echo pagination_bar($restaurants_count[0]->total, $per_page); ?>
            </div>                            
        </div>

        <script>
            //convert wp pagination to ajax pagination
            jQuery(function () {
                jQuery('.cp_paginate a').each(function () {
                    let myhref = jQuery(this).attr('href');
                    let ind = myhref.indexOf('paged=');
                    let pageNumber = myhref.substr(ind + 6);
                    jQuery(this).attr('href', 'javascript:void(0)');
                    jQuery(this).attr('onclick', 'cp_pagination(this,' + pageNumber + ')');
                });

            });//ends doc ready
            //AJAX PAGINATION
            function cp_pagination(obj, pageNumber) {
                jQuery(obj).addClass('current');
                //            ez_get_restaurents(obj, 'yes');
                let $this = jQuery(obj);
//                debugger;
                let the_data = jQuery('#ez_search_restaurants').serializeArray();
                let adminAjaxUrl = '<?php echo admin_url('admin-ajax.php'); ?>';
...
console.log(the_data);
                ...
                jQuery.ajax({
                    ...
                    jQuery("#current_page").val(pageNumber);
                    jQuery('#the_listings').html(response.SHtml);
