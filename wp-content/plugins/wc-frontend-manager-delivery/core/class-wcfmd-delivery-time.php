...
class WCFMd_Delivery_Time {
    ...
    global $WCFM, $WCFMmp, $wcfmd;	
		...
		$wcfm_delivery_time_enable          = isset( $wcfm_vendor_delivery_time['enable'] ) ? 'yes' : 'no';
		$wcfm_delivery_time_enable          = apply_filters( 'wcfmd_is_delivery_time_enable', $wcfm_delivery_time_enable, $vendor_id );
		...
		if( apply_filters( 'wcfm_is_allow_delivery_time', true ) ) {
			...
			if( !empty(  $wcfmd_delvery_times ) ) {
				foreach( $wcfmd_delvery_times as $vendor_id => $wcfmd_delvery_time ) {
					...
					echo __( 'Delivery Time', 'wc-frontend-manager-delivery' ).':</strong> ' . date_i18n( $time_format, $wcfmd_delvery_time ) . '</p>';
				}
			}
		}
	}
		...
		if( apply_filters( 'wcfm_is_allow_delivery_time', true ) ) {
			...
			if( !empty(  $wcfmd_delvery_times ) ) {
				foreach( $wcfmd_delvery_times as $vendor_id => $wcfmd_delvery_time ) {
					...
					$time_format = wcfm_delivery_time_display_format( $vendor_id );
					$shipping_address .= __( 'Delivery Time', 'wc-frontend-manager-delivery' ).':</strong> ' . date_i18n( $time_format, $wcfmd_delvery_time ) . '</p>';
				}
			}
		}
		...
		foreach ( $wcfmd_delvery_times as $vendor_id => $wcfmd_delvery_time ) {
			...
			<tr>
				<td colspan="3" style="background-color: #eeeeee;padding: 1em 1.41575em;line-height: 1.5;font-weight:600;">
					<?php 
					echo apply_filters( 'wcfm_delivery_time_order_details_label', $store_name, $vendor_id );
					?>
				</td>
				<td colspan="5" style="background-color: #f8f8f8;padding: 1em;"><?php echo date_i18n( $time_format, $wcfmd_delvery_time ); ?></td>
			</tr>
			<?php
		}
		echo "</tbody></table>";
