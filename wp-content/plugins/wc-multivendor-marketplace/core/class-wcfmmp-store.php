...
class WCFMmp_Store {
    ...
    public function get_store_tabs($with_count = true) {
        ...
        if (!WCFM_Dependencies::wcfmu_plugin_active_check() || !apply_filters('wcfm_is_pref_vendor_followers', true) || !apply_filters('wcfm_is_allow_store_followers', true) || !$WCFM->wcfm_vendor_support->wcfm_vendor_has_capability($this->id, 'vendor_follower')) {
            if (isset($store_tabs['followers']))
                unset($store_tabs['followers']);
        } else {
        ...
        if (!WCFM_Dependencies::wcfmu_plugin_active_check() || !apply_filters('wcfm_is_pref_vendor_followers', true) || !apply_filters('wcfm_is_allow_store_followings', false)) {
            if (isset($store_tabs['followings']))
                unset($store_tabs['followings']);
        } else {
        ...
        if (isset($store_tabs['policies']) && apply_filters('wcfmmp_is_allow_store_policy_tab_title_by_setting', true)) {
            ...
            if ($_wcfm_vendor_policy_tab_title)
                $store_tabs['policies'] = $_wcfm_vendor_policy_tab_title;
        }
        ...
        if (!apply_filters('wcfm_is_pref_vendor_reviews', true)) {
            ...
            if (isset($store_tabs['reviews']))
                unset($store_tabs['reviews']);
        } else {
    ...
    function get_customer_support_details() {
        ...
        if ($addr_1)
            $customer_support_details .= $addr_1;
        if ($addr_2)
            $customer_support_details .= ", " . $addr_2;
        if ($city)
            $customer_support_details .= ", " . $city;
        if ($state_name)
            $customer_support_details .= ", " . $state_name;
        if ($country_name)
            $customer_support_details .= " " . $country_name;
        if ($zip)
            $customer_support_details .= " - " . $zip;

        if ($email)
            $customer_support_details .= "<br/>" . __('Email', 'wc-multivendor-marketplace') . ': ' . $email;
        if ($phone)
            $customer_support_details .= "<br/>" . __('Phone', 'wc-multivendor-marketplace') . ': ' . $phone;
    ...
    public function get_list_banner_type() {
        ...
        if (( $list_banner_type == 'video' ) && empty($list_banner_video))
    ...
    public function get_banner_type() {
        ...
        if (( $banner_type == 'slider' ) && empty($banner_slider))
            $banner_type = 'single_img';
        if (( $banner_type == 'video' ) && empty($banner_video))
            $banner_type = 'single_img';
    ...
    public function get_last_review_author_id() {
        ...
        if (!$last_author_id)
            ...
            $last_author_id = absint($last_author_id);
